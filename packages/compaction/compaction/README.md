...
| Member | Semantics |
|---|---|
| `compactIfNeeded(agent, trigger, signal, options?)` | Consider automatic compaction for `trigger: 'pressure' \| 'context-overflow'`. A pressure trigger may apply the backend's threshold and retained-tail policy; a confirmed overflow may force a useful balanced reduction. An optional per-call `options.thresholdRatio` overrides the configured pressure threshold for that call only. Returns the `CompactionResult`, or `null` when no safe range exists. A backend's summarization request is a direct `ctx.llm.stream()` call (not a loop step), so per-call interception happens at `llm/stream`. |
