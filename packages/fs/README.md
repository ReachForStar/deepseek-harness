...
| `tool-excalidraw/` | Model-facing Excalidraw scene tools (`excalidraw_read`/`write`/`draw`/`export`) over the workspace scene file shared with the web canvas tab | (registers on `ctx.tools`) |
